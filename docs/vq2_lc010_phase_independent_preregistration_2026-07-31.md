# VQ2 LC010S phase-independent early stop — 2026-07-31
...
LC010R then reproduced the history exactly and composed the intended heads, but
its frozen epoch-0 head 8 differed from the stored fp32 baseline by only
`1.11e-8` absolute (`1.36e-5` relative), just outside its `1e-5` tolerance.
Under fresh tag `vq2_lc010s_phase_independent_early_stop_001`, replay LC009
exactly for six
...
512, no phase regression beyond `2e-5` relative numerical tolerance, at least
...
LC010S writes no classical runtime path, sends zero FlightSim packets, and has
