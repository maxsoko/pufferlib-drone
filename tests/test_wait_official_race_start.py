...
class RaceBeforeHeartbeatAdapter(FakeAdapter):
    def poll_telemetry(self, *, timeout_s=0.0):
        self.polls += 1
        self.telemetry.metrics.messages_seen += 1
        self.telemetry.metrics.race_statuses += 1
...
def test_wait_for_race_start_waits_for_heartbeat_fields():
    args = SimpleNamespace(
        endpoint="udpin:0.0.0.0:14550",
        duration=1.0,
        poll_timeout_s=0.0,
    ...
    adapter = RaceBeforeHeartbeatAdapter(args.endpoint)
    report = module.wait_for_race_start(args, adapter_factory=lambda *_a, **_kw: adapter)

    assert adapter.polls == 2
    assert report["race_started"] is True
    assert report["base_mode"] == 193
    assert report["system_status"] == 4
    assert report["heartbeats_seen"] == 1
