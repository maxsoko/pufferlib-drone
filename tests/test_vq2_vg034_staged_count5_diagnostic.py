...
ROOT = component.ROOT
PARENT_MANIFEST = ROOT / "docs/vq2_vg034_parent_count5_manifest_2026-07-31.json"
CANDIDATE_MANIFEST = (
    ROOT / "docs/vq2_vg034_candidate_count5_manifest_2026-07-31.json"
)
RUNNER = ROOT / "scripts/run_vq2_vg034_vast.sh"


def test_vg034_manifests_bind_same_fresh_count5_fixture() -> None:
    ...
    assert parent["seed"] == 429133
    assert parent["agents"] == parent["episodes"] == 32
    assert parent["num_threads"] == 4
    assert parent["max_steps"] == 2560


def test_vg034_runner_is_parallel_resumable_source_locked_and_offline() -> None:
    ...
    assert "tests/test_vq2_vg034_staged_count5_diagnostic.py" in text
