...
ROOT = component.ROOT
PARENT_MANIFEST = ROOT / "docs/vq2_vg043_parent_count5_manifest_2026-07-31.json"
CANDIDATE_MANIFEST = (
    ROOT / "docs/vq2_vg043_candidate_count5_manifest_2026-07-31.json"
)
RUNNER = ROOT / "scripts/run_vq2_vg043_vast.sh"


def test_vg043_manifests_bind_same_fresh_count5_fixture() -> None:
    ...
    assert parent["seed"] == 429157
    assert parent["agents"] == parent["episodes"] == 32
    assert parent["num_threads"] == 4
    assert parent["max_steps"] == 2560


def test_vg043_runner_is_parallel_resumable_source_locked_and_offline() -> None:
    ...
    assert "tests/test_vq2_vg043_staged_count5_diagnostic.py" in text
