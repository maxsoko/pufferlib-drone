...
SCRIPT = ROOT / "scripts" / "run_windows_vq2_gate2_policy_n476_shadow.ps1"


def test_n476_launcher_is_puffer_only_calibrated_shadow_and_hash_pinned():
    ...
    assert "PUFFER_POLICY_GATE2_INITIAL_GATE_POSITION_WORLD_SCALE_Z" in text
    assert '"--policy-shadow-only"' in text
    assert '"--no-arm-on-start"' in text
    assert '"--gate2-checkpoint"' in text
    assert "verify_live_policy_shadow.py" in text
