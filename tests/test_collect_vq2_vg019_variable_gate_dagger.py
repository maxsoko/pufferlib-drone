...
import scripts.collect_vq2_vg019_variable_gate_dagger as vg019
...
def test_vg019_frozen_candidate_rejection_and_oracle_verify() -> None:
    vg019.verify_inputs()


def test_vg019_configuration_collects_new_early_transition_states() -> None:
    collector_names = (
        "TAG",
        "SCHEMA",
        "STATE_SCHEMA",
    ...
    try:
        vg019.configure_collector()
        assert collector.TAG == vg019.TAG
        assert collector.SEED == 429093
        ...
        assert evaluator.CHECKPOINT_SHA256 == vg019.CHECKPOINT_SHA256
        source_paths = collector.collection_source_paths()
        for required in (
            Path(vg019.__file__).resolve(),
            vg019.PREREGISTRATION.resolve(),
            vg019.RUNNER.resolve(),
            vg019.CHECKPOINT.resolve(),
            vg019.VG018_REPORT.resolve(),
            vg019.VG018_REJECTION.resolve(),
        ):
        ...
        phase_records[:2] = (120_000, int(lengths.sum()) - 120_000)
        arguments = {
            "lengths": lengths,
...
def test_vg019_runner_is_source_locked_resumable_and_flightsim_free() -> None:
    text = vg019.RUNNER.read_text()
    assert os.access(vg019.RUNNER, os.X_OK)
    assert 'VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?' in text
    assert "git rev-parse HEAD" in text
    assert "git status --porcelain --untracked-files=no" in text
    assert vg019.TAG in text
    assert "tests/test_collect_vq2_vg019_variable_gate_dagger.py" in text
