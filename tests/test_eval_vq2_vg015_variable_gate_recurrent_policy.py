...
def test_vg015_candidate_and_admission_evidence_are_exact() -> None:
    vg015.verify_candidate()


def test_vg015_uses_fresh_fixed_course_seeds() -> None:
    assert tuple(vg015.SEEDS) == evaluator.COUNTS
    assert len(set(vg015.SEEDS.values())) == len(evaluator.COUNTS)
    assert set(vg015.SEEDS.values()).isdisjoint(evaluator.SEEDS.values())
    assert set(vg015.SEEDS.values()).isdisjoint(vg011.SEEDS.values())
    assert all(seed > 429065 for seed in vg015.SEEDS.values())


def test_vg015_keeps_crossing_margin_diagnostic_but_crash_hard(
...
def test_vg015_vast_runner_is_source_locked_resumable_and_offline() -> None:
    ...
    assert "vq2_vg015_variable_gate_recurrent_teacher_free_256" in text
    ...
    assert "tests/test_eval_vq2_vg015_variable_gate_recurrent_policy.py" in text
