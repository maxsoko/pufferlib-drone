...
def test_vg036_candidate_and_staged_admission_evidence_are_exact() -> None:
    vg036.verify_candidate()


def test_vg036_uses_fresh_fixed_course_seeds() -> None:
    assert tuple(vg036.SEEDS) == evaluator.COUNTS
    assert len(set(vg036.SEEDS.values())) == len(evaluator.COUNTS)
    for old in (evaluator.SEEDS, vg023.SEEDS, vg026.SEEDS, vg031.SEEDS):
        assert set(vg036.SEEDS.values()).isdisjoint(old.values())
    assert all(seed > 429134 for seed in vg036.SEEDS.values())


def test_vg036_retains_four_thread_terminal_episode_contract() -> None:
    old_seeds = evaluator.SEEDS
    try:
        evaluator.SEEDS = dict(vg036.SEEDS)
        config, overrides = vg036.fixed_thread_config(pufferl, num_gates=5)
    finally:
...
def test_vg036_keeps_crossing_margin_diagnostic_but_crash_hard(
...
def test_vg036_vast_runner_is_source_locked_resumable_and_offline() -> None:
    text = vg036.RUNNER.read_text()
    assert os.access(vg036.RUNNER, os.X_OK)
    assert 'VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?' in text
    assert "git rev-parse HEAD" in text
    assert "git status --porcelain --untracked-files=no" in text
    assert vg036.TAG in text
    ...
    assert "tests/test_eval_vq2_vg036_variable_gate_recurrent_policy.py" in text
