...
- As of 2026-07-03, aperture-based detection is stable and no longer reports clipping-related false passes, but `active_gate_index` remains `0` despite repeated vision-side gate events. The approach geometry blocker appears to be lateral correction timing (for example, low/right aperture alignment in the closest annotated frame). We added image-space roll steering (`--attitude-servo-k-image-roll`) with defaults that use full-frame normalization for tiny detections, and updated controller harnesses so the new arg is passed through safely in calibration/watch/race validation. A clean manual-reset sweep now runs via:
  `python scripts/run_sitl_control_calibration.py --skip-probe --no-reset-between-cases --attitude-servo-k-image-roll-sweep "0.0,-0.05,0.05,-0.12,0.12" --run-id ...`
  then tune sign and magnitude until first-gate `official_active_gate_index` advances.
