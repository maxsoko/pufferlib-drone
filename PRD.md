...
- As of 2026-07-03, aperture-based detection makes the local pass heuristic stricter and no longer records a false pass when the drone clips the red frame. The current blocker is the approach primitive: the drone reaches the first gate area but approaches low/right of the aperture, with the opening high-left in close debug frames. Next controller work should add image-space steering and/or calibrate a roll/lateral body-rate primitive before more native training or speed tuning.
