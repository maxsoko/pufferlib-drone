...
- Spec-compliant SITL baseline:
  ...
  - Next action: use clean simulator launch/manual race start, require simulator race-status advancement with `--require-official-race-progress`, and tune the controller until `active_gate_index` advances past `0`. Keep MAVLink auto-reset disabled for promotion runs until the black-frame/high-altitude reset behavior is explained.
...
- Visual-servo competition baseline:
  ...
  - Next action: use official race status as the validation judge; use `approach_diagnostics` to sweep command semantics and approach geometry. Highest-priority probes are local-NED yaw-active velocity axes, vertical sign/magnitude, and whether `SET_ATTITUDE_TARGET` or direct actuator control is required for reliable official gate traversal.
