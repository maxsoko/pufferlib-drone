...
- Visual-servo competition baseline:
  ...
  - Next action: use official race status as the validation judge; instrument controller runs with frame snapshots/pose/command samples near closest approach and retune visual-servo approach so the simulator records gate passage.
