#!/usr/bin/env python3
"""Collect a second phase-11 DAgger iteration from LC153."""
...
TAG = "vq2_lc155_phase11_failure_state_dagger2_001"
SCHEMA = "vq2_lc155_phase11_failure_state_dagger2_report_v1"
FEATURE_SCHEMA = "vq2_lc155_phase11_failure_state_dagger2_feature_v1"
PARENT_DIR = (
...
LC154_REPORT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc154_phase11_failure_state_dagger_milestone_001/report.json"
)
LC154_REPORT_SHA256 = "1b2062f408699320f65df8299a9fcf12605a5aa37f43442ce92a36609dd3b84a"
PREREGISTRATION = ROOT / "docs/vq2_lc155_phase11_failure_state_dagger2_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc155_vast.sh"
TEST = ROOT / "tests/test_collect_vq2_lc155_phase11_failure_state_dagger2.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        LC154_REPORT: LC154_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC155 bound input changed: {path}")
    parent = torch.load(PARENT_CHECKPOINT, map_location="cpu", weights_only=False)
    parent_report = json.loads(PARENT_REPORT.read_text())
    rejected = json.loads(LC154_REPORT.read_text())
    if (
        ...
        or rejected.get("schema") != "vq2_lc154_phase11_failure_state_dagger_milestone_report_v1"
        ...
        raise RuntimeError("LC153/LC154 do not authorize LC155")
    return parent
...
def source_identity() -> dict[str, Any]:
    ...
    paths = (
        ...
        PARENT_CHECKPOINT, PARENT_REPORT, LC154_REPORT,
...
def corrected_writer(path: Path, payload: dict[str, Any]) -> None:
    ...
    if path.name == "report.json" and corrected.get("schema") == SCHEMA:
        ...
        control["name"] = "lc153_puffer_failure_control"
        intervention["name"] = "lc153_phase11_alignment_oracle"
        ...
        predicates = {
            ...
            "all_rows_query_phase11": corrected["query_agents"] == prior.TOTAL_AGENTS,
            ...
            "only_phase11": bool(
                corrected["feature_phase_records"][prior.PHASE_MIN]
        ...
        corrected["actor_execution"] = (
            "two independent complete recurrent LC153 Puffer actors over 256 rows each"
        )
        corrected["next_authority"] = (
            "Fit the second phase-11 whole-Puffer DAgger residual; no FlightSim authority."
            if corrected["training_dataset_admitted"] else
            "Reject LC155; do not fit or run FlightSim."
        )
