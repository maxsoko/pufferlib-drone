param(
    [string]$Tag = "vq2_n482_fixed_state_shadow_004",
    [ValidateRange(8, 12)]
...
if ((Test-Path $JsonPath) -or (Test-Path $ParityPath)) {
    throw "refusing to overwrite existing N482 shadow evidence"
}
...
if ($LASTEXITCODE -ne 0) {
    ...
    throw "N482 fixed-state shadow failed"
}
...
if (-not $Report.acceptance_passed -or
    ...
    throw "N482 shadow acceptance or zero-command contract failed"
}
...
& $Python @VerifyArgs
if ($LASTEXITCODE -ne 0) {
    throw "N482 fixed-state shadow parity failed"
}
