#!/usr/bin/env python3
"""Teacher-free repeated-source raw-12 screen of LC153 against LC148."""
...
TAG = "vq2_lc154_phase11_failure_state_dagger_milestone_001"
SCHEMA = "vq2_lc154_phase11_failure_state_dagger_milestone_report_v1"
GROUP_SIZE = 128
PAIR_SIZE = 256
TARGET_PHASE = 11
TARGET_RAW_INDEX = 12
...
CANDIDATE_CHECKPOINT_SHA256 = "d7d379e5bdc7fe2489cfa6ca58aec443a27dcfe47b3cf6b422fb165e6dbb1ff9"
CANDIDATE_REPORT = CANDIDATE_DIR / "report.json"
CANDIDATE_REPORT_SHA256 = "7dc5cde27a9a854577e58dc4eee55766c8c7f42d6cd16cefe5f3d852299ae485"
SELECTED_PARAMETER_DELTA_L2 = 6.152730846083075
BIASES = (
    ("parent_lc148", (0.0, 0.0, 0.0, 0.0)),
    ("lc153_failure_state_dagger", (0.0, 0.0, 0.0, 0.0)),
)
PREREGISTRATION = ROOT / "docs/vq2_lc154_phase11_failure_state_dagger_milestone_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc154_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc154_phase11_failure_state_dagger_milestone.py"
...
def candidate_state_for_index(
    ...
    if candidate_index == 0:
        ...
        raise ValueError("LC154 has only parent and candidate")
...
def candidate_metadata_for_index(candidate_index: int) -> dict[str, Any]:
    delta = 0.0 if candidate_index == 0 else SELECTED_PARAMETER_DELTA_L2
    return {
        "phase11_parameter_delta_l2": delta,
        "bias_l2": delta,
        "checkpoint_sha256": (
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC154 bound input changed: {path}")
    ...
    if (
        ...
        or candidate.get("schema") != "vq2_lc153_phase11_failure_state_dagger_fit_checkpoint_v1"
        or not candidate.get("numerically_admitted")
        or candidate.get("parent_checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
        or training.get("schema") != "vq2_lc153_phase11_failure_state_dagger_fit_report_v1"
        ...
        or selected.get("success_improvement_factor", 0.0) < 306.0
        or training.get("safety", {}).get("flight_sim_packets_sent") != 0
        or training.get("safety", {}).get("submission_authorized")
    ):
        raise RuntimeError("LC148/LC153 do not authorize LC154")
    parent_state, candidate_state = parent["model_state"], candidate["model_state"]
    for name in parent_state:
        if name in PARAMETER_NAMES:
            keep = torch.arange(parent_state[name].shape[0]) != TARGET_PHASE
            if (
                not torch.equal(parent_state[name][keep], candidate_state[name][keep])
                or torch.equal(
                    parent_state[name][TARGET_PHASE], candidate_state[name][TARGET_PHASE]
                )
            ):
                raise RuntimeError(f"LC153 phase-11 isolation changed: {name}")
        elif not torch.equal(parent_state[name], candidate_state[name]):
            raise RuntimeError(f"LC153 changed frozen state: {name}")
    return parent
...
def configure() -> None:
    ...
    milestone.NEXT_AUTHORITY_SELECTED = (
        "Use LC153 only as the source-trajectory parent for phase-12 offline search; independent admission remains unproven."
    )
    milestone.NEXT_AUTHORITY_NONE = (
        "Reject LC153 and run another phase-11 on-policy DAgger iteration; do not run FlightSim."
    )
    milestone.VECTORIZED_CHECKPOINT_SURGERY = (
        "two complete saved-form Puffer actors over 256 identical seed-15 rows; LC153 changes only the four phase-11 residual parameter rows"
    )
