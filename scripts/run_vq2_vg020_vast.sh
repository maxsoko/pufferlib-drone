#!/usr/bin/env bash
# Source-locked Vast.ai bootstrap and epoch-resumable VG020 refit.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT to the pushed source commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg020_variable_gate_five_source_refit_001}"
...
VQ2_DAGGER4="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg019_variable_gate_dagger_round4_vg017_visited_512"
VQ2_LEGACY_PARENT="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg014_variable_gate_three_source_refit_001"
VQ2_PARENT="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg017_variable_gate_four_source_refit_001"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG020 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ "$VQ2_CPU_COUNT" -lt 32 ]; then
    echo "VG020 requires at least 32 visible CPUs" >&2
    exit 2
fi
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG020 requires approximately 64 GiB RAM" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 5 ]; then
    echo "VG020 requires at least 5 GiB free after dataset staging" >&2
    exit 2
fi
...
test "$(sha256sum "$VQ2_DAGGER4/report.json" | cut -d" " -f1)" = \
    2079ce9de0ff96bab8e218ba249307988799a7c7062d7175185d86608c213e82
test "$(sha256sum "$VQ2_DAGGER4/metadata.json" | cut -d" " -f1)" = \
...
test "$(sha256sum docs/vq2_vg019_variable_gate_dagger_round4_admission_2026-07-30.json | cut -d" " -f1)" = \
    9b110d50fb4f36c750725eb0f2f77afc8506d6eaeab739577b10e02aa715b286
test "$(sha256sum "$VQ2_LEGACY_PARENT/policy_best.pt" | cut -d" " -f1)" = \
    60d69a1f80a1551b190cd4b517dfa029bc3dbe48728a853a47b658df0866d79e
test "$(sha256sum "$VQ2_LEGACY_PARENT/report.json" | cut -d" " -f1)" = \
...
if not torch.cuda.is_available():
    raise SystemExit("VG020 cannot see CUDA")
...
if not any(candidate in name for candidate in ("RTX 4090", "A100", "RTX A6000")):
    raise SystemExit(f"unsupported VG020 GPU: {name}")
