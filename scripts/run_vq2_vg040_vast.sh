#!/usr/bin/env bash
# Source-locked Vast bootstrap and epoch-resumable VG040 nine-source refit.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG040 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg040_variable_gate_nine_source_refit_001}"
...
run_training() {
    ...
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 \
        python scripts/train_vq2_variable_gate_nine_source_refit.py \
            --output "$VQ2_OUTPUT" \
            --device cuda \
            "${resume[@]}"
}
...
if [ ! -x .venv/bin/python ]; then
    echo "VG040 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ "$(nproc)" -lt 32 ]; then
    echo "VG040 requires at least 32 visible CPUs" >&2
    exit 2
fi
...
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG040 requires approximately 64 GiB RAM" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
    echo "VG040 requires at least 15 GiB free" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG040 cannot see CUDA")
