#!/usr/bin/env python3
"""Teacher-free repeated-source raw-16 screen of LC169 against LC166."""
...
TAG = "vq2_lc170_phase15_failure_state_dagger2_milestone_001"
SCHEMA = "vq2_lc170_phase15_failure_state_dagger2_milestone_report_v1"
...
CANDIDATE_DIR = ROOT / "logs/drone_race_full_policy_six_gate_bootstrap/vq2_lc169_phase15_failure_state_dagger2_fit_001"
CANDIDATE_CHECKPOINT = CANDIDATE_DIR / "policy_selected.pt"
CANDIDATE_CHECKPOINT_SHA256 = "8ae1a6d9aebd01d584344f006724f5a81699a011a759356283ada1f271ece1b7"
CANDIDATE_REPORT = CANDIDATE_DIR / "report.json"
CANDIDATE_REPORT_SHA256 = "182e81da9cb41a7f8479f0a2cfa99baef9b408a63cdfd1024130e38349cccf9b"
SELECTED_PARAMETER_DELTA_L2 = 2.7561824584048114
BIASES = (
    ("parent_lc166", (0.0, 0.0, 0.0, 0.0)),
    ("lc169_failure_state_dagger2", (0.0, 0.0, 0.0, 0.0)),
)
PREREGISTRATION = ROOT / "docs/vq2_lc170_phase15_failure_state_dagger2_milestone_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc170_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc170_phase15_failure_state_dagger2_milestone.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC170 bound input changed: {path}")
    ...
    if (
        ...
        or candidate.get("schema") != "vq2_lc169_phase15_failure_state_dagger2_fit_checkpoint_v1"
        or not candidate.get("numerically_admitted")
        or candidate.get("parent_checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
        or training.get("schema") != "vq2_lc169_phase15_failure_state_dagger2_fit_report_v1"
        ...
        or selected.get("success_improvement_factor", 0.0) < 1156.6
        or training.get("safety", {}).get("flight_sim_packets_sent") != 0
        or training.get("safety", {}).get("submission_authorized")
    ):
        raise RuntimeError("LC166/LC169 do not authorize LC170")
    for name in parent["model_state"]:
        if name in PARAMETER_NAMES:
            ...
            if (
                ...
                or torch.equal(
                    parent["model_state"][name][previous.TARGET_PHASE],
                    candidate["model_state"][name][previous.TARGET_PHASE],
                )
            ):
                raise RuntimeError(f"LC169 phase-15 isolation changed: {name}")
        elif not torch.equal(parent["model_state"][name], candidate["model_state"][name]):
            raise RuntimeError(f"LC169 changed frozen state: {name}")
    return parent
...
def configure() -> None:
    ...
    previous.previous.prior.milestone.NEXT_AUTHORITY_SELECTED = (
        "Use LC169 only as the phase-16 source-trajectory parent; broad admission remains mandatory."
    )
    previous.previous.prior.milestone.NEXT_AUTHORITY_NONE = (
        "Reject LC169 and continue phase-15 state-dependent optimization; do not run FlightSim."
    )
    previous.previous.prior.milestone.VECTORIZED_CHECKPOINT_SURGERY = (
        "two complete saved-form Puffer actors over 256 identical seed-15 rows; LC169 changes only phase-15 residual parameter rows"
    )
