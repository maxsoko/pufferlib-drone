#!/usr/bin/env bash
# Source-locked Vast bootstrap and resumable VG061 direct-head bracket.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG061 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg061_vg060_direct_phase4_residual_bracket_001}"
...
VQ2_UPDATE="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg060_direct_phase4_indexed_residual_001"
VQ2_PARENT_ADMISSION="$VQ2_WORKSPACE_PATH/docs/vq2_vg033_eight_source_refit_admission_2026-07-31.json"
VQ2_UPDATE_ADMISSION="$VQ2_WORKSPACE_PATH/docs/vq2_vg060_direct_phase4_indexed_residual_admission_2026-07-31.json"
...
run_screen() {
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 \
        python scripts/eval_vq2_vg061_direct_phase4_residual_bracket.py \
...
test "$(sha256sum "$VQ2_UPDATE/policy_best.pt" | cut -d" " -f1)" = \
    32aba1580844d20b51ddf7930892bf5367d79a44ec0d6d4d5e316fdf10246a26
test "$(sha256sum "$VQ2_UPDATE/report.json" | cut -d" " -f1)" = \
    68f5295328f8453824f3e3fa3dc6e0053afcf68ad0f5757fdeaebf95ce083f82
test "$(sha256sum "$VQ2_UPDATE_ADMISSION" | cut -d" " -f1)" = \
    13f6c301f208c616f8c28e5287a07d2e8cf17934e242e9b2c0db5d888075f1c6
...
OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 python -m pytest -q \
    tests/test_eval_vq2_vg061_direct_phase4_residual_bracket.py \
