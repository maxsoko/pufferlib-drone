#!/usr/bin/env bash
# Source-locked Vast.ai bootstrap and resumable VG011 teacher-free screen.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT to the pushed source commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg011_variable_gate_recurrent_teacher_free_256}"
VQ2_STATE="$VQ2_OUTPUT/state.json"
VQ2_CANDIDATE="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg010_variable_gate_source_balanced_refit_001"
VQ2_ADMISSION="$VQ2_WORKSPACE_PATH/docs/vq2_vg010_variable_gate_dagger_refit_admission_2026-07-30.json"

run_vg011_screen() {
    python scripts/eval_vq2_vg011_variable_gate_recurrent_policy.py \
        --output "$VQ2_OUTPUT" \
        --device cuda \
        --resume
}
...
if [ ! -x .venv/bin/python ]; then
    echo "VG011 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ "$VQ2_CPU_COUNT" -lt 32 ]; then
    echo "VG011 requires at least 32 visible CPUs; found $VQ2_CPU_COUNT" >&2
    exit 2
fi
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG011 requires approximately 64 GiB RAM; found ${VQ2_RAM_GIB} GiB" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
    echo "VG011 requires at least 15 GiB free; found ${VQ2_DISK_FREE_GIB} GiB" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG011 cannot see CUDA")
...
if not any(candidate in name for candidate in ("RTX 4090", "A100", "RTX A6000")):
    raise SystemExit(f"unsupported VG011 GPU: {name}")
