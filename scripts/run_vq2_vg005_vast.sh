#!/usr/bin/env bash
# Source-locked Vast.ai launcher and epoch-resumable VG005 recurrent BC fit.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT to the pushed source commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg005_variable_gate_recurrent_bc_001}"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG005 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if not any(candidate in name for candidate in ("RTX 4090", "A100", "RTX A6000")):
    raise SystemExit(f"unsupported VG005 GPU: {name}")
