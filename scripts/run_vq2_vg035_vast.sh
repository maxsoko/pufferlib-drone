#!/usr/bin/env bash
# Source-locked paired count-11 diagnostic for VG028 versus VG033.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG035 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg035_vg033_paired_count11_diagnostic_001}"
VQ2_PARENT_OUTPUT="$VQ2_OUTPUT/parent"
VQ2_CANDIDATE_OUTPUT="$VQ2_OUTPUT/candidate"
VQ2_REPORT="$VQ2_OUTPUT/report.json"
VQ2_PARENT_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg035_parent_count11_manifest_2026-07-31.json"
VQ2_CANDIDATE_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg035_candidate_count11_manifest_2026-07-31.json"
VQ2_PREREGISTRATION="$VQ2_WORKSPACE_PATH/docs/vq2_vg035_paired_count11_diagnostic_preregistration_2026-07-31.md"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG035 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ ! -f "$VQ2_PARENT_OUTPUT/state.json" ] \
    ...
    if [ "$(nproc)" -lt 32 ]; then
        echo "VG035 requires at least 32 visible CPUs" >&2
        exit 2
    fi
    ...
    if [ "$VQ2_RAM_GIB" -lt 60 ]; then
        echo "VG035 requires approximately 64 GiB RAM" >&2
        exit 2
    fi
    if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
        echo "VG035 requires at least 15 GiB free" >&2
        exit 2
    fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG035 cannot see CUDA")
...
PY
    ...
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 python -m pytest -q \
        ...
        tests/test_vq2_vg035_staged_count11_diagnostic.py \
...
if [ "$parent_exit" -ne 0 ] || [ "$candidate_exit" -ne 0 ]; then
    echo "VG035 component failed before paired comparison" >&2
    exit 2
fi
