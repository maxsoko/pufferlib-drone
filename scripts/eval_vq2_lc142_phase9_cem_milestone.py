#!/usr/bin/env python3
"""Teacher-free paired raw-10 screen of LC141 against frozen LC105."""
...
TAG = "vq2_lc142_phase9_cem_milestone_001"
SCHEMA = "vq2_lc142_phase9_cem_milestone_report_v1"
GROUP_SIZE = 128
TARGET_PHASE = 9
TARGET_RAW_INDEX = 10
MAX_STEPS = 12_000
SEED = 432_150
MINIMUM_PASS_GAIN = 1
PARENT_CHECKPOINT = pairwise.PARENT_CHECKPOINT
PARENT_CHECKPOINT_SHA256 = pairwise.PARENT_CHECKPOINT_SHA256
PARENT_REPORT = pairwise.PARENT_REPORT
PARENT_REPORT_SHA256 = pairwise.PARENT_REPORT_SHA256
CANDIDATE_DIR = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc141_phase9_split_batch_cem_001"
)
CANDIDATE_CHECKPOINT = CANDIDATE_DIR / "policy_selected.pt"
CANDIDATE_CHECKPOINT_SHA256 = "33d8b837fe6c94c522179191e0c869e7b24a4839b96b96bfed5f5d09b0b057da"
CANDIDATE_REPORT = CANDIDATE_DIR / "report.json"
CANDIDATE_REPORT_SHA256 = "ba24a298edb46fb2ef25bf643c6733c0fce49314e74a4991c933896a444c7306"
SELECTED_DELTA = (
    -0.2931601405143738,
    0.1841394007205963,
    0.03680568188428879,
    0.007408940698951483,
)
BIASES = (
    ("baseline_lc105", (0.0, 0.0, 0.0, 0.0)),
    ("lc141_phase9_cem", SELECTED_DELTA),
)
PREREGISTRATION = ROOT / "docs/vq2_lc142_phase9_cem_milestone_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc142_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc142_phase9_cem_milestone.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC142 bound input changed: {path}")
    parent = torch.load(PARENT_CHECKPOINT, map_location="cpu", weights_only=False)
    parent_report = json.loads(PARENT_REPORT.read_text())
    candidate = candidate_payload()
    training = json.loads(CANDIDATE_REPORT.read_text())
    selected = training.get("selected_candidate", {})
    if (
        ...
        or candidate.get("schema") != "vq2_lc141_phase9_split_batch_cem_checkpoint_v1"
        or not candidate.get("numerically_admitted")
        or candidate.get("parent_checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
        or not candidate.get("frozen_non_phase9_state_exact")
        or tuple(candidate.get("phase9_pre_tanh_output_bias_delta", ())) != SELECTED_DELTA
        or training.get("schema") != "vq2_lc141_phase9_split_batch_cem_report_v1"
        or not training.get("training_admitted")
        or training.get("candidate_selected_for_screen", {}).get("sha256")
        != CANDIDATE_CHECKPOINT_SHA256
        or selected.get("delta") != list(SELECTED_DELTA)
        or training.get("generations", [{}, {}])[1].get("target_passes") != 2
        or training.get("safety", {}).get("flight_sim_packets_sent") != 0
        or training.get("safety", {}).get("submission_authorized")
    ):
        raise RuntimeError("LC105/LC141 do not authorize LC142")
    expected = candidate_state_for_index(parent["model_state"], 1)
    if any(not torch.equal(expected[name], candidate["model_state"][name]) for name in expected):
        raise RuntimeError("LC142 candidate state reconstruction changed")
    return parent
...
def configure() -> None:
    ...
    milestone.NEXT_AUTHORITY_SELECTED = (
        "Run a larger independent deterministic LC141 robustness screen; no FlightSim authority."
    )
    milestone.NEXT_AUTHORITY_NONE = (
        "Reject LC141 and retain LC105; do not run FlightSim."
    )
    milestone.VECTORIZED_CHECKPOINT_SURGERY = (
        "two complete saved-form Puffer actors, each executing a 256-row paired batch; LC141 differs only at phase 9"
    )
