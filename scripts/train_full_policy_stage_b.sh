#!/usr/bin/env bash
# Full-policy Stage B: learn official-start gate 1 with no FSM flight commands.
set -euo pipefail
...
# 4096 agents x 1800 steps is 7.37M transitions per complete episode. Use a
# chunk large enough for every agent to observe terminal credit at least twice.
REPO_ROOT="$ROOT" \
ENV_NAME=drone_race_full_policy \
CHUNK_STEPS="${CHUNK_STEPS:-16000000}" \
