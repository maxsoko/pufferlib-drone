#!/usr/bin/env bash
# Run six source-locked concurrent PuffeRL workers on the retained Vast host.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT_ROOT="${VQ2_OUTPUT_ROOT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_lc006_multiworker_puffer_throughput_001}"
...
if [ -e "$VQ2_OUTPUT_ROOT" ]; then
    echo "refusing to overwrite LC006 evidence" >&2
    exit 2
fi
...
OMP_NUM_THREADS=1 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=1 python -m pytest -q \
    tests/test_benchmark_vq2_lc003_native_puffer_training.py \
    tests/test_benchmark_vq2_lc006_multiworker_puffer_training.py

OMP_DYNAMIC=FALSE python \
    scripts/benchmark_vq2_lc006_multiworker_puffer_training.py \
    --baseline "$VQ2_BASELINE" --output-root "$VQ2_OUTPUT_ROOT"
