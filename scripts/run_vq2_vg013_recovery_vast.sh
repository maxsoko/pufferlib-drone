#!/usr/bin/env bash
# Source-locked deterministic recovery of the finalized VG012 corpus.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT to the pushed source commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_DATASET="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg012_variable_gate_dagger_round2_vg010_visited_512"
VQ2_ORIGINAL_STATE="${VQ2_DATASET}_state.json"
VQ2_ORIGINAL_LOG="${VQ2_ORIGINAL_LOG:-/workspace/vq2_vg012_collection_001.log}"
VQ2_ARCHIVE="${VQ2_ARCHIVE:-/workspace/vq2_vg012_failed_postcollection_corpus.tgz}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg013_vg012_postcollection_recovery_001}"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG013 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
source .venv/bin/activate

if [ -f "$VQ2_STATE" ]; then
    python scripts/recover_vq2_vg012_postcollection.py \
        --output "$VQ2_OUTPUT" \
        --dataset "$VQ2_DATASET" \
        --original-state "$VQ2_ORIGINAL_STATE" \
        --run-log "$VQ2_ORIGINAL_LOG" \
        --archive "$VQ2_ARCHIVE" \
        --device cuda \
        --resume
    exit $?
...
if [ "$(nproc)" -lt 32 ]; then
    echo "VG013 requires at least 32 visible CPUs" >&2
    exit 2
fi
VQ2_DISK_FREE_GIB="$(df -BG --output=avail "$VQ2_WORKSPACE_PATH" | tail -1 | tr -dc '0-9')"
if [ "$VQ2_DISK_FREE_GIB" -lt 10 ]; then
    echo "VG013 requires at least 10 GiB free" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG013 cannot see CUDA")
...
OMP_NUM_THREADS=16 MKL_NUM_THREADS=16 python -m pytest -q \
    ...
    tests/test_collect_vq2_vg012_variable_gate_dagger.py \
    tests/test_recover_vq2_vg012_postcollection.py \
