#!/usr/bin/env python3
"""Held-out validation of the N725 multi-horizon prior child."""
...
from scripts.continue_vq2_prior_multihorizon_offline import CONTINUATION_SCHEMA
...
AUDIT_SCHEMA = "vq2_multihorizon_prior_heldout_validation_v1"
N725_CHECKPOINT_SHA256 = (
    "8330d75e8c47378c5182b5d47994e4456e36a72f7928b6897505801260dd99ed"
)
N725_REPORT_SHA256 = "bb8ef01907c4d85b058a3341f0dbdd5d77f6df655d649ec9182b7302fc21b418"
...
def _improvement_gates(
    deltas: dict[str, dict[str, float]], *, suffix: str
) -> dict[str, bool]:
    return {
        f"h{horizon}_open_vs_posterior_mae_improved_{suffix}": (
...
def audit(args: argparse.Namespace) -> dict[str, object]:
    ...
    source_before = {
        ...
        "n725_report": _sha256(args.n725_report),
    ...
    expected = {
        "checkpoint": N725_CHECKPOINT_SHA256,
        "n725_report": N725_REPORT_SHA256,
    ...
    if any(source_before[name] != digest for name, digest in expected.items()):
        raise RuntimeError("N726 source hash mismatch")
    n725 = json.loads(args.n725_report.read_text(encoding="utf-8"))
    ...
    if (
        n725.get("contract") != CONTINUATION_SCHEMA
        or n725.get("process_passed") is not True
        or n725.get("prior_updates") != 1
        or n725.get("output_checkpoint_sha256") != N725_CHECKPOINT_SHA256
        ...
        raise RuntimeError("N726 predecessor contract mismatch")
    ...
    if not process_passed:
        ...
        raise RuntimeError(f"N726 process gates failed: {failed}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n725-report", type=Path, required=True)
    ...
    if args.report.exists():
        parser.error("N726 refuses to overwrite its report")
    return args
