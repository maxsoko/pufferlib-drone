#!/usr/bin/env bash
# Source-locked Vast bootstrap and resumable 32-thread VG026 screen.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG026 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg026_variable_gate_recurrent_teacher_free_256}"
...
VQ2_PARITY="$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg026_screen_thread_parity_001/report.json"

run_vg026_screen() {
    python scripts/eval_vq2_vg026_variable_gate_recurrent_policy.py \
        --output "$VQ2_OUTPUT" \
        --device cuda \
        --resume
}
...
if [ ! -x .venv/bin/python ]; then
    echo "VG026 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ "$(nproc)" -lt 32 ]; then
    echo "VG026 requires at least 32 visible CPUs" >&2
    exit 2
fi
...
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG026 requires approximately 64 GiB RAM" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
    echo "VG026 requires at least 15 GiB free" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG026 cannot see CUDA")
...
python scripts/check_vq2_vg026_screen_thread_parity.py \
    --output "$VQ2_PARITY" \
    --device cuda
python - "$VQ2_PARITY" <<'PY'
import json
import sys
report = json.load(open(sys.argv[1]))
if not report.get("admitted") or report.get("threads") != [4, 32]:
    raise SystemExit("VG026 thread parity did not admit")
if report.get("optimizer_steps") != 0 or report.get("state_writes") != 0:
    raise SystemExit("VG026 thread parity changed policy state")
PY
