#!/usr/bin/env bash
# N211: fine-tune N209 exclusively from the measured official Gate-4 entry
# distribution, with fixed-course spline teacher shaping on roll and thrust.
# The teacher is reward-only; deployment remains one recurrent 32-input policy.
set -euo pipefail
...
SOURCE_CHECKPOINT="${SOURCE_CHECKPOINT:-logs/drone_race_full_policy_six_gate_bootstrap/n209_measured_entry_mixture/checkpoints/drone_race_full_policy_six_gate_bootstrap/1784409692380/0000000000589824.bin}"
...
if [[ ! -f "$SOURCE_CHECKPOINT" ]]; then
    echo "missing N209 parent: $SOURCE_CHECKPOINT" >&2
    exit 2
fi
...
python -m pufferlib.pufferl train drone_race_full_policy_six_gate_bootstrap \
    ...
    --eval-episodes 0 \
    --tag n211_measured_tail_teacher \
    --seed 211 \
