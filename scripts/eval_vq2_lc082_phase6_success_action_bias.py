#!/usr/bin/env python3
"""Screen phase-6 biases measured from successful Puffer trajectories."""
...
TAG = "vq2_lc082_phase6_success_action_bias_001"
SCHEMA = "vq2_lc082_phase6_success_action_bias_report_v1"
GROUP_SIZE = 32
TARGET_PHASE = 6
TARGET_RAW_INDEX = 7
MAX_STEPS = 12_000
BIASES: tuple[tuple[str, tuple[float, float, float, float]], ...] = (
    ...
    ("roll_m0p010", (0.0, -0.010, 0.0, 0.0)),
    ("roll_m0p025", (0.0, -0.025, 0.0, 0.0)),
    ("roll_m0p050", (0.0, -0.050, 0.0, 0.0)),
    ("thrust_p0p010", (0.0, 0.0, 0.010, 0.0)),
    ("thrust_p0p025", (0.0, 0.0, 0.025, 0.0)),
...
LC079_FEATURES = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc079_phase6_uncensored_outcomes_001/features.bin"
)
LC079_FEATURES_SHA256 = "2a1dcf1be9c7e983a3e5e36a62a5bad36571d9ca0175d2ddde014760047fb4be"
LC081_REPORT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc081_phase6_success_endpoint_milestone_001/report.json"
)
LC081_REPORT_SHA256 = "d3299d8b233c74c761b33e4e9cfed766cb13ea28c1bbb29f082c0892bca4d941"
PREREGISTRATION = ROOT / "docs/vq2_lc082_phase6_success_action_bias_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc082_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc082_phase6_success_action_bias.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        LC079_FEATURES: LC079_FEATURES_SHA256,
        LC081_REPORT: LC081_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC082 bound input changed: {path}")
    parent_report = json.loads(PARENT_REPORT.read_text())
    rejected = json.loads(LC081_REPORT.read_text())
    import torch
    ...
    if (
        ...
        or rejected.get("schema") != "vq2_lc081_phase6_success_endpoint_milestone_report_v1"
...
def configure() -> None:
    ...
    base.EXTRA_SOURCE_PATHS = (Path(__file__).resolve(), LC079_FEATURES, LC081_REPORT)
    base.NEXT_AUTHORITY_SELECTED = (
        "Run one larger different-seed raw-index-7 confirmation of the selected success-action bias."
    )
    base.NEXT_AUTHORITY_NONE = "Reject the successful-action bias family and retain LC073."
    base.VECTORIZED_CHECKPOINT_SURGERY = (
        "constant phase-6 indexed Puffer output bias measured from successful Puffer trajectories"
    )
