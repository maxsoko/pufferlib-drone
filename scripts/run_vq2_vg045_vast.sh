#!/usr/bin/env bash
# Source-locked paired confirmation for VG033 versus VG044 alpha 0.10.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG045 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg045_vg044_paired_count5_confirmation_001}"
VQ2_PARENT_OUTPUT="$VQ2_OUTPUT/parent"
VQ2_CANDIDATE_OUTPUT="$VQ2_OUTPUT/candidate"
VQ2_REPORT="$VQ2_OUTPUT/report.json"
VQ2_PARENT_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg045_parent_count5_manifest_2026-07-31.json"
VQ2_CANDIDATE_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg045_candidate_count5_manifest_2026-07-31.json"
VQ2_PREREGISTRATION="$VQ2_WORKSPACE_PATH/docs/vq2_vg045_paired_count5_confirmation_preregistration_2026-07-31.md"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG045 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
run_comparator() {
    ...
    python scripts/compare_vq2_vg045_paired_count5_confirmation.py \
...
if [ ! -f "$VQ2_PARENT_OUTPUT/state.json" ] \
    ...
    if [ "$(nproc)" -lt 32 ]; then
        echo "VG045 requires at least 32 visible CPUs" >&2
        exit 2
    fi
    ...
    if [ "$VQ2_RAM_GIB" -lt 60 ]; then
        echo "VG045 requires approximately 64 GiB RAM" >&2
        exit 2
    fi
    if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
        echo "VG045 requires at least 15 GiB free" >&2
        exit 2
    fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG045 cannot see CUDA")
...
PY
    ...
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 python -m pytest -q \
        ...
        tests/test_vq2_vg045_paired_count5_confirmation.py \
...
if [ "$parent_exit" -ne 0 ] || [ "$candidate_exit" -ne 0 ]; then
    echo "VG045 component failed before paired comparison" >&2
    exit 2
fi
