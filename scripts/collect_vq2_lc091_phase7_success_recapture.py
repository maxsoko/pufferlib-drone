#!/usr/bin/env python3
"""Recapture LC087's source-locked successful phase-7 trajectories."""
...
TAG = "vq2_lc091_phase7_success_recapture_001"
SCHEMA = "vq2_lc091_phase7_success_recapture_report_v1"
STATE_SCHEMA = "vq2_lc091_phase7_success_recapture_state_v1"
AGENTS = EPISODES = 128
THREADS = 32
SEED = 431_870
TARGET_PHASE = 7
...
EXPECTED_QUERY_AGENTS = 3
EXPECTED_SUCCESS_AGENTS = 2
EXPECTED_FAILURE_AGENTS = 1
PARENT_DIR = (
...
LC090_REPORT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc090_phase7_uncensored_outcomes_001/report.json"
)
LC090_REPORT_SHA256 = "1010366c427bf57d2d0418f89a8ce05212853386b9a6a0f64642d8e0f0347834"
PREREGISTRATION = ROOT / "docs/vq2_lc091_phase7_success_recapture_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc091_vast.sh"
TEST = ROOT / "tests/test_collect_vq2_lc091_phase7_success_recapture.py"
...
def verify_inputs() -> None:
    for path, digest in {
        ...
        LC090_REPORT: LC090_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC091 bound input changed: {path}")
    parent = json.loads(PARENT_REPORT.read_text())
    failed = json.loads(LC090_REPORT.read_text())
    ...
    if (
        ...
        or failed.get("schema") != "vq2_lc090_phase7_uncensored_outcomes_report_v1"
        or failed.get("training_dataset_admitted")
        or failed.get("query_agents") != 4
        or failed.get("query_outcome_success_agents") != 0
        or failed.get("query_outcome_failure_agents") != 4
        or failed.get("safety", {}).get("flight_sim_packets_sent") != 0
        or failed.get("safety", {}).get("submission_authorized")
    ):
        raise RuntimeError("LC087/LC090 do not authorize LC091")
...
def configure() -> None:
    ...
    base.EXTRA_SOURCE_PATHS = (
        Path(__file__).resolve(), TEST, PARENT_REPORT, LC090_REPORT,
    )
