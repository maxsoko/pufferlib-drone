#!/usr/bin/env python3
"""Verify N172's late one-sided Gate-2 residual roll floor."""
...
CLEAN_GATE2_CANDIDATES = {
    ...
    "024", "025", "026", "027", "028", "030", "031",
}
EXERCISED_FAILURES = {"015", "017", "023", "032"}
...
def analyze(path: Path) -> dict:
    ...
    for sample_index, sample in enumerate(samples):
        ...
        governed = np.asarray(
            hybrid._gate2_late_residual_roll_floor(
                values,
                recorded.tolist(),
            ),
            dtype=float,
    ...
    return {
        ...
        "roll_floor_violations": sum(
            ...
            != hybrid.GATE2_LATE_RESIDUAL_ROLL_FLOOR_NORM
            for row in rows
        ),
...
def main() -> None:
    ...
    parser.add_argument("--candidate032-poststop", required=True, type=Path)
    ...
    blockers: list[str] = []
    for candidate in sorted(CLEAN_GATE2_CANDIDATES | EXERCISED_FAILURES):
    ...
    for candidate in sorted(EXERCISED_FAILURES):
        report = by_candidate.get(candidate)
        if report and report["changed_samples"] < 1:
            blockers.append(f"candidate{candidate}_residual_floor_not_exercised")

    candidate32 = by_candidate.get("032")
    if candidate32 is not None:
        if candidate32["changed_samples"] != 2:
            blockers.append("candidate032_change_count_not_two")
        forward_values = [row["forward_m"] for row in candidate32["rows"]]
        if forward_values and not (
            2.4 <= min(forward_values)
            <= max(forward_values)
            <= 3.2
        ):
            blockers.append("candidate032_activation_range_changed")
    ...
    poststop = json.loads(args.candidate032_poststop.read_text(encoding="utf-8"))
    race = ((poststop.get("latest_telemetry") or {}).get("race_status") or {})
    if poststop.get("reset_sent") is not False:
        blockers.append("candidate032_poststop_not_command_free")
    if int(race.get("active_gate_index", -1)) != 2:
        blockers.append("candidate032_poststop_does_not_prove_gate2")
    if int(race.get("race_finish_time_ns", -2)) != -1:
        blockers.append("candidate032_poststop_finish_unexpected")
    ...
    payload = {
        ...
        "contract": {
            ...
            "max_forward_m": hybrid.GATE2_LATE_RESIDUAL_MAX_FORWARD_M,
            "projected_right_trigger_m": (
                hybrid.GATE2_LATE_RESIDUAL_PROJECTED_RIGHT_TRIGGER_M
            ),
            "normalized_roll_floor": hybrid.GATE2_LATE_RESIDUAL_ROLL_FLOOR_NORM,
            "positive_projected_miss_only": True,
            "preserve_pitch_thrust_yaw_and_stronger_roll": True,
        },
        "candidate032_poststop": {
            "path": str(args.candidate032_poststop),
            "sha256": _sha256(args.candidate032_poststop),
