#!/usr/bin/env python3
"""N196: exact N195 composition with later Gate-3 terminal roll leveling."""
...
class Gate3LaterTerminalSafeLevel:
    """Level roll only after a safe Gate-3 projection inside 3.7 metres."""
...
_LATER_TERMINAL = Gate3LaterTerminalSafeLevel()
...
def infer(observation: Sequence[float]) -> list[float]:
    ...
    actions = _LATER_TERMINAL.apply(sanitized, actions)
    actions = _RAPID_VERTICAL.apply(sanitized, actions)
    return _FROZEN_RECOVERY.apply(sanitized, actions)


def controller_snapshot() -> dict:
    return {
        "gate2_frozen_observation_dropout": _SANITIZER.snapshot(),
        "gate3_later_terminal_safe_level": _LATER_TERMINAL.snapshot(),
