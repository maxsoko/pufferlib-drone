#!/usr/bin/env bash
# Run the source-locked LC003 native PuffeRL throughput benchmark on Vast.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set VQ2_EXPECTED_COMMIT}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT_ROOT="${VQ2_OUTPUT_ROOT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_lc003_native_puffer_throughput_001}"
...
if [ -e "$VQ2_OUTPUT_ROOT" ]; then
    echo "refusing to overwrite LC003 evidence" >&2
    exit 2
fi
...
OMP_DYNAMIC=FALSE python scripts/benchmark_vq2_lc003_native_puffer_training.py \
