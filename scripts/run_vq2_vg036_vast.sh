#!/usr/bin/env bash
# Source-locked Vast bootstrap and resumable VG036 teacher-free screen.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG036 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg036_vg033_variable_gate_recurrent_teacher_free_256}"
...
run_vg036_screen() {
    python scripts/eval_vq2_vg036_variable_gate_recurrent_policy.py \
        --output "$VQ2_OUTPUT" \
        --device cuda \
        --resume
}
...
if [ ! -x .venv/bin/python ]; then
    echo "VG036 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ "$(nproc)" -lt 32 ]; then
    echo "VG036 requires at least 32 visible CPUs" >&2
    exit 2
fi
...
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG036 requires approximately 64 GiB RAM" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
    echo "VG036 requires at least 15 GiB free" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG036 cannot see CUDA")
