#!/usr/bin/env bash
# Deterministic native evaluation of one VQ2 Gate-1 PufferLib checkpoint.
# Set PERTURBED=1 for the source-disjoint robustness contract.
set -euo pipefail
...
if [[ "$PERTURBED" == 1 ]]; then
    extra=(
        ...
        --env.gate-position-jitter-x 0.25
        --env.gate-position-jitter-y 0.15
        --env.gate-position-jitter-z 0.20
