#!/usr/bin/env bash
# Source-locked paired count-5 diagnostic for VG028 versus VG033.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG034 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg034_vg033_paired_count5_diagnostic_001}"
VQ2_PARENT_OUTPUT="$VQ2_OUTPUT/parent"
VQ2_CANDIDATE_OUTPUT="$VQ2_OUTPUT/candidate"
VQ2_REPORT="$VQ2_OUTPUT/report.json"
VQ2_PARENT_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg034_parent_count5_manifest_2026-07-31.json"
VQ2_CANDIDATE_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg034_candidate_count5_manifest_2026-07-31.json"
VQ2_PREREGISTRATION="$VQ2_WORKSPACE_PATH/docs/vq2_vg034_paired_count5_diagnostic_preregistration_2026-07-31.md"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG034 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ ! -f "$VQ2_PARENT_OUTPUT/state.json" ] \
    ...
    if [ "$(nproc)" -lt 32 ]; then
        echo "VG034 requires at least 32 visible CPUs" >&2
        exit 2
    fi
    ...
    if [ "$VQ2_RAM_GIB" -lt 60 ]; then
        echo "VG034 requires approximately 64 GiB RAM" >&2
        exit 2
    fi
    if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
        echo "VG034 requires at least 15 GiB free" >&2
        exit 2
    fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG034 cannot see CUDA")
...
PY
    ...
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 python -m pytest -q \
        ...
        tests/test_vq2_vg034_staged_count5_diagnostic.py \
...
if [ "$parent_exit" -ne 0 ] || [ "$candidate_exit" -ne 0 ]; then
    echo "VG034 component failed before paired comparison" >&2
    exit 2
fi
