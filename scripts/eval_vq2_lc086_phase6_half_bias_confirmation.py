#!/usr/bin/env python3
"""Confirm the selected half-scale phase-6 success-action bias."""
...
TAG = "vq2_lc086_phase6_half_bias_confirmation_001"
SCHEMA = "vq2_lc086_phase6_half_bias_confirmation_report_v1"
GROUP_SIZE = 128
TARGET_PHASE = 6
TARGET_RAW_INDEX = 7
MAX_STEPS = 12_000
...
BIASES = (
    ...
    ("lc085_success_direction_a0p5", SELECTED_BIAS),
)
SEED = 431860
MINIMUM_PASS_GAIN = 2
PARENT_DIR = (
...
LC085_REPORT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc085_phase6_success_bias_scale_bracket_001/report.json"
)
LC085_REPORT_SHA256 = "923dcab36a13b445cc7e7803a0a64af15e14765ac21029677cfa36df56437db5"
PREREGISTRATION = ROOT / "docs/vq2_lc086_phase6_half_bias_confirmation_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc086_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc086_phase6_half_bias_confirmation.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        LC085_REPORT: LC085_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC086 bound input changed: {path}")
    parent_report = json.loads(PARENT_REPORT.read_text())
    screen = json.loads(LC085_REPORT.read_text())
    ...
    if (
        ...
        or screen.get("schema") != "vq2_lc085_phase6_success_bias_scale_bracket_report_v1"
        ...
        raise RuntimeError("LC073/LC085 do not authorize LC086")
    return parent
...
def configure() -> None:
    ...
    base.EXTRA_SOURCE_PATHS = (Path(__file__).resolve(), LC085_REPORT)
    base.NEXT_AUTHORITY_SELECTED = (
        "Run one paired 24-gate full-course screen of the confirmed half-bias."
    )
    base.NEXT_AUTHORITY_NONE = "Reject the selected half-bias and retain LC073."
    base.VECTORIZED_CHECKPOINT_SURGERY = (
        "exact half-scale phase-6 Puffer output bias measured from successful trajectories"
    )
