#!/usr/bin/env bash
# Source-locked paired count-5 diagnostic for VG025 versus VG028.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG029 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg029_vg028_paired_count5_diagnostic_001}"
VQ2_PARENT_OUTPUT="$VQ2_OUTPUT/parent"
VQ2_CANDIDATE_OUTPUT="$VQ2_OUTPUT/candidate"
VQ2_REPORT="$VQ2_OUTPUT/report.json"
VQ2_PARENT_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg029_parent_count5_manifest_2026-07-31.json"
VQ2_CANDIDATE_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg029_candidate_count5_manifest_2026-07-31.json"
VQ2_PREREGISTRATION="$VQ2_WORKSPACE_PATH/docs/vq2_vg029_paired_count5_diagnostic_preregistration_2026-07-31.md"
...
if [ ! -x .venv/bin/python ]; then
    echo "VG029 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
...
if [ ! -f "$VQ2_PARENT_OUTPUT/state.json" ] \
    ...
    if [ "$(nproc)" -lt 32 ]; then
        echo "VG029 requires at least 32 visible CPUs" >&2
        exit 2
    fi
    ...
    if [ "$VQ2_RAM_GIB" -lt 60 ]; then
        echo "VG029 requires approximately 64 GiB RAM" >&2
        exit 2
    fi
    if [ "$VQ2_DISK_FREE_GIB" -lt 15 ]; then
        echo "VG029 requires at least 15 GiB free" >&2
        exit 2
    fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG029 cannot see CUDA")
...
if [ "$parent_exit" -ne 0 ] || [ "$candidate_exit" -ne 0 ]; then
    echo "VG029 component failed before paired comparison" >&2
    exit 2
fi
