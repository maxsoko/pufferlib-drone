#!/usr/bin/env python3
"""Bracket amplitudes along the confirmed phase-6 success-action bias."""
...
TAG = "vq2_lc085_phase6_success_bias_scale_bracket_001"
SCHEMA = "vq2_lc085_phase6_success_bias_scale_bracket_report_v1"
GROUP_SIZE = 32
TARGET_PHASE = 6
TARGET_RAW_INDEX = 7
MAX_STEPS = 12_000
...
BIASES = tuple(
    (
        "baseline_lc073" if scale == 0.0 else f"success_direction_a{str(scale).replace('.', 'p')}",
...
LC084_REPORT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_lc084_phase6_success_bias_full_course_001/report.json"
)
LC084_REPORT_SHA256 = "1b7987992f4c7319baadbb83dd3c3c0bebb9facc8565b5250db6b810aa758176"
PREREGISTRATION = ROOT / "docs/vq2_lc085_phase6_success_bias_scale_bracket_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc085_vast.sh"
TEST = ROOT / "tests/test_eval_vq2_lc085_phase6_success_bias_scale_bracket.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        LC084_REPORT: LC084_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC085 bound input changed: {path}")
    parent_report = json.loads(PARENT_REPORT.read_text())
    confirmation = json.loads(LC083_REPORT.read_text())
    rejected = json.loads(LC084_REPORT.read_text())
    import torch
    ...
    if (
        ...
        or tuple(selected.get("phase6_output_bias", ())) != DIRECTION
        or selected.get("paired_target_gains_vs_baseline") != 3
        or selected.get("paired_target_losses_vs_baseline") != 0
        or rejected.get("schema") != "vq2_lc084_phase6_success_bias_full_course_report_v1"
        ...
        raise RuntimeError("LC073/LC083/LC084 do not authorize LC085")
    return parent
...
def configure() -> None:
    ...
    base.EXTRA_SOURCE_PATHS = (
        Path(__file__).resolve(), LC083_REPORT, LC084_REPORT,
    )
    base.NEXT_AUTHORITY_SELECTED = (
        "Run one larger different-seed raw-index-7 confirmation of the selected scale."
    )
    base.NEXT_AUTHORITY_NONE = "Reject the phase-6 success-direction scale family and retain LC073."
    base.VECTORIZED_CHECKPOINT_SURGERY = (
        "scaled constant phase-6 indexed Puffer output bias measured from successful trajectories"
    )
