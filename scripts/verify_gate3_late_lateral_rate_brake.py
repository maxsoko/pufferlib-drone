#!/usr/bin/env python3
"""Verify the N171 late Gate-3 lateral-rate brake on live traces."""
...
def analyze(path: Path) -> dict:
    ...
    return {
        ...
        "roll_output_violations": sum(
            row["governed_action"][1] != hybrid.GATE3_LATE_RATE_BRAKE_ROLL_NORM
            for row in rows
        ),
...
def main() -> None:
    ...
    parser.add_argument("--candidate031-poststop", required=True, type=Path)
    ...
    elif candidate30["changed_samples"] != 0:
        blockers.append("candidate030_margin_path_changed")
    candidate31 = by_candidate.get("031")
    if candidate31 is None:
        blockers.append("candidate031_missing")
    elif candidate31["changed_samples"] != 2:
        blockers.append("candidate031_change_count_not_two")
    else:
        ranges = [row["forward_m"] for row in candidate31["rows"]]
        if not (
            hybrid.GATE3_LATE_RATE_BRAKE_MIN_FORWARD_M < min(ranges)
            <= max(ranges)
            <= hybrid.GATE3_LATE_RATE_BRAKE_MAX_FORWARD_M
        ):
            blockers.append("candidate031_change_ranges_unexpected")
    ...
    for report in reports:
        ...
        if report["roll_output_violations"]:
            blockers.append(f"candidate{report['candidate']}_roll_output_violation")
        if report["positive_roll_changes"]:
            blockers.append(f"candidate{report['candidate']}_changes_positive_roll")

    poststop = json.loads(args.candidate031_poststop.read_text(encoding="utf-8"))
    race = ((poststop.get("latest_telemetry") or {}).get("race_status") or {})
    if poststop.get("reset_sent") is not False:
        blockers.append("candidate031_poststop_not_command_free")
    if int(race.get("race_start_boot_time_ms", -2)) != -1:
        blockers.append("candidate031_poststop_not_inactive_after_collision")
    if int(race.get("race_finish_time_ns", -2)) != -1:
        blockers.append("candidate031_poststop_finish_unexpected")
    ...
    payload = {
        ...
        "contract": {
            ...
            "left_side_only": True,
            "preserve_pitch_thrust_yaw_and_positive_roll": True,
        },
        "candidate031_poststop": {
            "path": str(args.candidate031_poststop),
            "sha256": _sha256(args.candidate031_poststop),
