#!/usr/bin/env python3
"""Verify N178's earlier Gate-3 brake and exact margin-floor priority."""
...
def main() -> None:
    ...
    parser.add_argument("--candidate038-poststop", required=True, type=Path)
    ...
    candidate38 = by_candidate.get("038")
    if candidate38 is not None:
        rows = candidate38["rows"]
        ranges = [row["forward_m"] for row in rows]
        recorded_rolls = [row["recorded_action"][1] for row in rows]
        if not (
            len(rows) == 2
            and 4.5 < min(ranges) <= max(ranges) <= 6.0
            and recorded_rolls.count(
                hybrid.GATE3_TERMINAL_MARGIN_ROLL_FLOOR_NORM
            )
            == 1
            and recorded_rolls.count(0.0) == 1
        ):
            blockers.append("candidate038_priority_samples_unexpected")
    ...
    for report in reports:
        ...
        expected_positive = 1 if candidate == "038" else 0
        ...
        if candidate == "038":
            for row in report["rows"]:
                recorded_roll = row["recorded_action"][1]
                if recorded_roll > 0.0 and (
                    recorded_roll
                    != hybrid.GATE3_TERMINAL_MARGIN_ROLL_FLOOR_NORM
                ):
                    blockers.append("candidate038_overrides_nonmargin_positive_roll")

    poststop = json.loads(args.candidate038_poststop.read_text(encoding="utf-8"))
    latest = poststop.get("latest_telemetry") or {}
    race = latest.get("race_status") or {}
    if poststop.get("reset_sent") is not False:
        blockers.append("candidate038_poststop_not_command_free")
    if int(race.get("race_start_boot_time_ms", -2)) != -1:
        blockers.append("candidate038_poststop_race_not_inactive")
    if int(race.get("race_finish_time_ns", -2)) != -1:
        blockers.append("candidate038_poststop_finish_unexpected")
    if int(latest.get("base_mode", -1)) != 65:
        blockers.append("candidate038_poststop_not_disarmed")
    ...
    payload = {
        ...
        "contract": {
            ...
            "override_exact_terminal_margin_floor": (
                hybrid.GATE3_TERMINAL_MARGIN_ROLL_FLOOR_NORM
            ),
            "preserve_pitch_thrust_yaw_and_other_positive_roll": True,
        },
        "candidate038_poststop": {
            "path": str(args.candidate038_poststop),
            "sha256": _sha256(args.candidate038_poststop),
