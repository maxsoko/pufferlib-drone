#!/usr/bin/env python3
"""Verify N180's bounded Gate-3 brake continuity across center."""
...
def main() -> None:
    ...
    parser.add_argument("--candidate040-poststop", required=True, type=Path)
    ...
    candidate40 = by_candidate.get("040")
    if candidate40 is not None:
        rows = candidate40["rows"]
        if not (
            len(rows) == 1
            and 0.0 < rows[0]["right_m"] <= hybrid.GATE3_LATE_RATE_BRAKE_MAX_RIGHT_M
            and rows[0]["right_rate_m_s"] >= hybrid.GATE3_LATE_RATE_BRAKE_MIN_RIGHT_RATE_M_S
            and abs(rows[0]["projected_right_at_terminal_m"])
            <= hybrid.GATE3_LATE_RATE_BRAKE_MAX_ABS_PROJECTED_RIGHT_M
            and rows[0]["recorded_action"][1] == 0.0
        ):
            blockers.append("candidate040_cross_center_sample_unexpected")
    ...
    poststop = json.loads(args.candidate040_poststop.read_text(encoding="utf-8"))
    latest = poststop.get("latest_telemetry") or {}
    race = latest.get("race_status") or {}
    if poststop.get("reset_sent") is not False:
        blockers.append("candidate040_poststop_not_command_free")
    if int(race.get("active_gate_index", -1)) != 2:
        blockers.append("candidate040_poststop_gate_index_not_two")
    if int(race.get("last_gate_race_time", -1)) != 7329575061:
        blockers.append("candidate040_poststop_gate_time_mismatch")
    if int(race.get("race_finish_time_ns", -2)) != -1:
        blockers.append("candidate040_poststop_finish_unexpected")
    if int(latest.get("base_mode", -1)) != 65:
        blockers.append("candidate040_poststop_not_disarmed")
    ...
    payload = {
        ...
        "contract": {
            ...
            "max_current_right_m": hybrid.GATE3_LATE_RATE_BRAKE_MAX_RIGHT_M,
        ...
        "candidate040_poststop": {
            "path": str(args.candidate040_poststop),
            "sha256": _sha256(args.candidate040_poststop),
