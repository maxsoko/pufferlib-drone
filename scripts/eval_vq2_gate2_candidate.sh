#!/usr/bin/env bash
# Deterministic two-gate native evaluation using the Gate-2 position inferred
# from N295's legal camera trace. Set PERTURBED=1 for source-disjoint jitter.
set -euo pipefail
...
if [[ "$PERTURBED" == 1 ]]; then
    extra=(
        --env.start-x-jitter 0.15
        --env.start-y-jitter 0.15
        --env.start-z-jitter 0.15
        --env.start-vx-jitter 0.10
        --env.start-vy-jitter 0.10
        --env.start-vz-jitter 0.10
        --env.start-roll-jitter-rad 0.02
        --env.start-pitch-jitter-rad 0.02
        --env.start-yaw-jitter-rad 0.02
        --env.start-wx-jitter 0.02
        --env.start-wy-jitter 0.02
        --env.start-wz-jitter 0.02
        ...
        --env.gate-position-jitter-x 0.35
...
python scripts/eval_drone_race_checkpoint.py "$CHECKPOINT" \
    ...
    --env.start-vx 0 --env.start-vy 0 --env.start-vz 0 \
    ...
    --env.gate0-x 10.78 --env.gate0-y 0.084 --env.gate0-z 0.56 \
    --env.gate1-x 25.13 --env.gate1-y 8.96 --env.gate1-z 1.65 \
