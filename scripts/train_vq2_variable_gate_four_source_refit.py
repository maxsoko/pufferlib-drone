#!/usr/bin/env python3
"""Refit VG014 with clean and three successively visited DAgger sources."""
...
TAG = "vq2_vg017_variable_gate_four_source_refit_001"
CHECKPOINT_SCHEMA = "vq2_variable_gate_recurrent_bc_checkpoint_v1"
REPORT_SCHEMA = "vq2_variable_gate_four_source_refit_report_v1"
STATE_SCHEMA = "vq2_variable_gate_four_source_refit_state_v1"
SEED = 429080
...
PREREGISTRATION = (
    ROOT / "docs/vq2_vg017_four_source_refit_preregistration_2026-07-30.md"
)
RUNNER = ROOT / "scripts/run_vq2_vg017_vast.sh"
DEFAULT_OUTPUT = (
...
class FourSourceConfig:
    ...
    clean_objective_weight: float = 0.40
    dagger1_objective_weight: float = 0.15
    dagger2_objective_weight: float = 0.15
    dagger3_objective_weight: float = 0.30
...
def objective_weights(config: FourSourceConfig) -> dict[str, float]:
    return {
...
def four_source_loss(
    ...
    *,
    ...
    if total is None:
        raise RuntimeError("VG017 has no source objective")
    return total, components


def source_weight_audit(
    sums: dict[str, float], updates: int, config: FourSourceConfig
) -> bool:
    expected = {
...
def verify_inputs() -> None:
    ...
    for path, digest in expected.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"VG017 source evidence hash mismatch: {path}")
    if not PREREGISTRATION.is_file():
        raise RuntimeError("VG017 preregistration is missing")
    ...
    if (
        parent.get("schema") != "vq2_variable_gate_three_source_refit_report_v1"
        or parent.get("tag") != "vq2_vg014_variable_gate_three_source_refit_001"
        or not parent.get("completed")
        or not parent.get("numerically_admitted")
        or parent.get("checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
    ):
        raise RuntimeError("VG014 parent is not admitted for VG017")
    if (
        ...
        raise RuntimeError("VG014 admission evidence does not bind VG017")
    if (
        ...
        raise RuntimeError("recovered VG012 is not admitted for VG017")
    if (
        dagger3.get("schema")
    ...
    if (
        ...
        raise RuntimeError("VG016 admission evidence does not bind VG017")
...
def _load_parent(model: VQ2PhaseRecurrentActor) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("tag") != "vq2_vg014_variable_gate_three_source_refit_001"
        ...
        raise RuntimeError("VG014 parent actor contract changed")
...
def verify_completed_output(output: Path, report: dict[str, Any]) -> None:
    if (
        ...
        raise RuntimeError("existing VG017 report identity changed")
    ...
    if (
        ...
        raise RuntimeError("completed VG017 source/checkpoint identity changed")
    state = torch.load(
    ...
    if (
        ...
        raise RuntimeError("completed VG017 state does not bind the report")
...
def train(
    *,
    ...
    config: FourSourceConfig = FourSourceConfig(),
    resume: bool = False,
) -> dict[str, Any]:
    if config.sequence_chunk < 256 or config.transition_window_exposure < 3:
        raise RuntimeError("VG017 requires 256-step BPTT and 3x transitions")
    if objective_weights(config) != SOURCE_WEIGHTS:
        raise RuntimeError("VG017 requires exact 0.40/0.15/0.15/0.30 weights")
    verify_inputs()
    if device_name == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("VG017 preregisters CUDA training")
    ...
    if output.exists() and not state_path.is_file():
        raise RuntimeError("VG017 output exists without resumable state")
    ...
    if state_path.is_file():
        if not resume:
            raise FileExistsError(f"VG017 state already exists at {state_path}")
        saved = torch.load(state_path, map_location=device, weights_only=False)
        for key, expected in identity.items():
            if saved.get(key) != expected:
                raise RuntimeError(f"VG017 resume mismatch for {key}")
        if saved.get("status") != "training":
            raise RuntimeError("VG017 can resume only active training state")
        ...
        if resume:
            raise RuntimeError("VG017 --resume requested without state")
    ...
    for epoch in range(completed_epoch + 1, config.epochs + 1):
        ...
        while True:
            ...
            for name in ("dagger1", "dagger2", "dagger3"):
                item = streams[name].next(model)
                if item is None:
                    raise RuntimeError(f"VG017 cyclic {name} stream ended")
                items[name] = item
            ...
            for _ in range(repetitions):
                ...
                with torch.autocast(
                    ...
                    loss, components = four_source_loss(
            ...
            if set(final_outputs) != set(datasets):
                raise RuntimeError("VG017 emitted no complete source update")
            for name, item in items.items():
        ...
        if not weights_exact:
            raise RuntimeError("VG017 source objective weights diverged")
    ...
    if minimum_transition_exposure + 1e-12 < config.transition_window_exposure:
        raise RuntimeError("VG017 transition exposure contract failed")
    if not source_balance_audit:
        raise RuntimeError("VG017 source balance audit failed")
    selected_validation = (
    ...
    numerically_admitted = bool(
        ...
        < baseline_validation["dagger3"]["weighted_mse"]
