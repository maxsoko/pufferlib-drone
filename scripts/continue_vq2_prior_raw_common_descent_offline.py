#!/usr/bin/env python3
"""Apply the single N730-selected raw common-descent prior step."""
...
CONTINUATION_SCHEMA = "vq2_prior_raw_common_descent_single_update_v1"
...
N730_REPORT_SHA256 = "c4bf0fa8c4ecd92ddca64b7490736ba55628b5c90302a1ad67c023be3d0612bd"
LEARNING_RATE = 0.01


@torch.no_grad()
def _posterior_actions(model, legal, action, initial) -> torch.Tensor:
...
def continue_prior(args: argparse.Namespace) -> dict[str, object]:
    if args.checkpoint.resolve() == args.output_checkpoint.resolve():
        raise ValueError("raw common-descent continuation must not overwrite its parent")
    ...
    source_before = {
        ...
        "n730_report": _sha256(args.n730_report),
    ...
    expected = {
        ...
        "n730_report": N730_REPORT_SHA256,
    ...
    if first_legal is None or first_action is None or first_initial is None:
        raise RuntimeError("raw common-descent schedule is empty")
    ...
    weights = torch.as_tensor(direction["weights"], device=device, dtype=torch.float64)
    ...
    process_gates = {
        ...
        "n730_baseline_parity": baseline_parity <= 1e-6,
        "n730_virtual_metric_parity": virtual_parity <= 1e-6,
    ...
    report: dict[str, object] = {
        ...
        "n730_baseline_parity_max_abs_error": baseline_parity,
        "n730_virtual_metric_parity_max_abs_error": virtual_parity,
    ...
    if not report["process_passed"]:
        ...
        raise RuntimeError(f"raw common-descent continuation gates failed: {failed}")
    output = dict(payload)
    output["model"] = child_state
    output["offline_raw_common_descent_prior_updates"] = 1
    output["offline_raw_common_descent_prior_parent_sha256"] = source_before["checkpoint"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n730-report", type=Path, required=True)
    ...
    if args.output_checkpoint.exists() or args.report.exists():
        parser.error("raw common-descent continuation refuses to overwrite outputs")
    return args
