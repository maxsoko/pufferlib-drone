#!/usr/bin/env python3
"""Collect phase-15 oracle labels on LC181 adapter-owned trajectories."""
...
TAG = "vq2_lc183_phase15_adapter_onpolicy_dagger_001"
SCHEMA = "vq2_lc183_phase15_adapter_onpolicy_dagger_report_v1"
FEATURE_SCHEMA = "vq2_lc183_phase15_adapter_onpolicy_dagger_feature_v1"
GROUP_SIZE = 256
TOTAL_AGENTS = 512
MAX_STEPS = 27_000
TARGET_RAW_INDEX = 16
...
LC182_REPORT = ROOT / "logs/drone_race_full_policy_six_gate_bootstrap/vq2_lc182_phase15_recurrent_adapter_convergence_milestone_001/report.json"
LC182_REPORT_SHA256 = "71af2e473f825f280657ba9a5a6d3caaddcbfbc498f3fdaf84554a2cbefc2339"
PREREGISTRATION = ROOT / "docs/vq2_lc183_phase15_adapter_onpolicy_dagger_preregistration_2026-08-01.md"
RUNNER = ROOT / "scripts/run_vq2_lc183_vast.sh"
TEST = ROOT / "tests/test_collect_vq2_lc183_phase15_adapter_onpolicy_dagger.py"
...
def verify_inputs() -> dict[str, Any]:
    for path, digest in {
        ...
        LC182_REPORT: LC182_REPORT_SHA256,
    }.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"LC183 bound input changed: {path}")
    parent = torch.load(PARENT_CHECKPOINT, map_location="cpu", weights_only=False)
    parent_report = json.loads(PARENT_REPORT.read_text())
    rejected = json.loads(LC182_REPORT.read_text())
    if (
        parent.get("schema") != "vq2_lc181_phase15_recurrent_adapter_final_convergence_checkpoint_v1"
        or not parent.get("numerically_admitted")
        or parent.get("model", {}).get("adapter_target_phase") != PHASE_MIN
        or parent.get("model", {}).get("adapter_size") != 64
        ...
        or rejected.get("schema") != "vq2_lc182_phase15_recurrent_adapter_convergence_milestone_report_v1"
        ...
        raise RuntimeError("LC181/LC182 do not authorize LC183")
    return parent
...
def adapter_feature_components(
    ...
    """Exclude adapter state/output while labeling its on-policy trajectory."""

    if next_recurrent.shape != (1, TOTAL_AGENTS, 320):
        raise RuntimeError("LC183 adapter recurrent ABI changed")
    residuals = []
    for group, child in enumerate(actor.actors):
        selected = slice(group * GROUP_SIZE, (group + 1) * GROUP_SIZE)
        residuals.append(
            child.phase_adapter_output(next_recurrent[0, selected, 256:])
        )
    adapter_residual = torch.cat(residuals, dim=0)
    return (
        next_recurrent[0, :, :256].index_select(0, index),
        (result.pre_tanh_mean - adapter_residual).index_select(0, index),
    )
...
def source_identity() -> dict[str, Any]:
    ...
    paths = (
        ...
        PARENT_CHECKPOINT, PARENT_REPORT, LC182_REPORT,
...
def corrected_writer(path: Path, payload: dict[str, Any]) -> None:
    ...
    if path.name == "report.json" and corrected.get("schema") == SCHEMA:
        ...
        control["name"] = "lc181_adapter_onpolicy_control"
        intervention["name"] = "lc181_adapter_onpolicy_oracle"
        ...
        predicates = {
            ...
            "only_phase15": corrected["feature_phase_records"][PHASE_MIN] == corrected["feature_records"],
        ...
        corrected["feature_base_pre_tanh_contract"] = "frozen LC169 whole-Puffer output before LC181 adapter residual"
        ...
        corrected["actor_execution"] = "two independent complete recurrent LC181 adapter Puffers over 256 rows each"
        corrected["next_authority"] = (
            "Fit the phase-15 recurrent adapter on LC181-owned failure and rescue sequences, then screen raw 16; no FlightSim authority."
            if corrected["training_dataset_admitted"] else
            "Reject LC183; do not fit or run FlightSim."
        )
