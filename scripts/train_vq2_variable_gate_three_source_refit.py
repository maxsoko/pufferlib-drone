#!/usr/bin/env python3
"""Refit VG010 with VG003, VG009, and recovered VG012 source balance."""
...
TAG = "vq2_vg014_variable_gate_three_source_refit_001"
CHECKPOINT_SCHEMA = "vq2_variable_gate_recurrent_bc_checkpoint_v1"
REPORT_SCHEMA = "vq2_variable_gate_three_source_refit_report_v1"
STATE_SCHEMA = "vq2_variable_gate_three_source_refit_state_v1"
SEED = 429065
...
DAGGER1_ADMISSION_SHA256 = (
    "71c4898f7a7a306b0f6def06bda63d5192de456c8a849e1f08f83ba908cd3091"
)
DAGGER2_DATASET = (
    ROOT
    / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_vg012_variable_gate_dagger_round2_vg010_visited_512"
)
DAGGER2_METADATA_SHA256 = (
    "e7bfdba11d1a1c65187c820203bb3a675d51b93cdff9270498ce8280aa25caac"
)
DAGGER2_RECOVERY_REPORT = (
    ROOT
    / "logs/drone_race_full_policy_six_gate_bootstrap"
    / "vq2_vg013_vg012_postcollection_recovery_001/report.json"
)
DAGGER2_RECOVERY_REPORT_SHA256 = (
    "38358a5813b7d6811d186006bc44a2e4470e51ad5a3d9c462bef4516b2b069c9"
)
DAGGER2_ADMISSION = (
    ROOT / "docs/vq2_vg013_postcollection_recovery_admission_2026-07-30.json"
)
DAGGER2_ADMISSION_SHA256 = (
...
PREREGISTRATION = (
    ROOT / "docs/vq2_vg014_three_source_refit_preregistration_2026-07-30.md"
)
RUNNER = ROOT / "scripts/run_vq2_vg014_vast.sh"
DEFAULT_OUTPUT = (
    ROOT / "logs/drone_race_full_policy_six_gate_bootstrap" / TAG
)
SOURCE_WEIGHTS = {"clean": 0.50, "dagger1": 0.25, "dagger2": 0.25}


@dataclass(frozen=True)
class ThreeSourceConfig:
    seed: int = SEED
    hidden_size: int = 256
    ...
    clean_objective_weight: float = 0.50
    dagger1_objective_weight: float = 0.25
    dagger2_objective_weight: float = 0.25
...
def three_source_loss(
    ...
    *,
    ...
    config: ThreeSourceConfig,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    """Normalize each source independently, then apply exact fixed weights."""
    ...
    for name, source_weight in objective_weights.items():
    ...
    if total is None:
        raise RuntimeError("VG014 has no source objective")
    return total, components


def source_weight_audit(
    sums: dict[str, float], updates: int, config: ThreeSourceConfig
) -> bool:
    expected = {
        "clean": updates * config.clean_objective_weight,
        "dagger1": updates * config.dagger1_objective_weight,
        "dagger2": updates * config.dagger2_objective_weight,
    }
...
def verify_inputs() -> None:
    ...
    for path, digest in expected.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"VG014 source evidence hash mismatch: {path}")
    if not PREREGISTRATION.is_file():
        raise RuntimeError("VG014 preregistration is missing")
    parent_report = json.loads(PARENT_REPORT.read_text())
    recovered = json.loads(DAGGER2_RECOVERY_REPORT.read_text())
    admission = json.loads(DAGGER2_ADMISSION.read_text())
    if (
        not parent_report.get("completed")
        or not parent_report.get("numerically_admitted")
        or parent_report.get("checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
    ):
        raise RuntimeError("VG010 parent is not admitted for VG014")
    if (
        recovered.get("schema") != "vq2_vg012_postcollection_recovery_report_v1"
        or not recovered.get("completed")
        or not recovered.get("admitted")
        or recovered.get("dataset", {}).get("records") != 171_903
        or recovered.get("failed_recovery_predicates")
    ):
        raise RuntimeError("VG013 recovery report does not admit VG012")
    if (
        admission.get("schema")
        != "vq2_vg013_postcollection_recovery_admission_v1"
        or not admission.get("admitted")
        or admission.get("artifact_sha256", {}).get("recovery_report")
        != DAGGER2_RECOVERY_REPORT_SHA256
    ):
        raise RuntimeError("VG013 admission evidence does not bind VG014")
...
def _load_parent(model: VQ2PhaseRecurrentActor) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("tag") != "vq2_vg010_variable_gate_source_balanced_refit_001"
        ...
        raise RuntimeError("VG010 parent actor contract changed")
...
def verify_completed_output(output: Path, report: dict[str, Any]) -> None:
    if (
        ...
        raise RuntimeError("existing VG014 report identity changed")
    ...
    if (
        ...
        raise RuntimeError("completed VG014 source/checkpoint identity changed")
    state = torch.load(
    ...
    if (
        ...
        raise RuntimeError("completed VG014 state does not bind the report")
...
def train(
    *,
    ...
    config: ThreeSourceConfig = ThreeSourceConfig(),
    resume: bool = False,
) -> dict[str, Any]:
    if config.sequence_chunk < 256 or config.transition_window_exposure < 3:
        raise RuntimeError("VG014 requires 256-step BPTT and 3x transitions")
    configured_weights = (
        config.clean_objective_weight,
        config.dagger1_objective_weight,
        config.dagger2_objective_weight,
    )
    if configured_weights != (0.50, 0.25, 0.25):
        raise RuntimeError("VG014 requires exact 0.50/0.25/0.25 source weights")
    verify_inputs()
    if device_name == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("VG014 preregisters CUDA training")
    ...
    if output.exists() and not state_path.is_file():
        raise RuntimeError("VG014 output exists without resumable state")
    ...
    splits = {
        name: _dataset_split(dataset.agents, split_counts[name])
    ...
    identity = {
        ...
        "source_balancing": {
            ...
            "training_agents": {
                name: len(splits[name][0]) for name in datasets
            },
            "validation_agents": {
    ...
    if state_path.is_file():
        if not resume:
            raise FileExistsError(f"VG014 state already exists at {state_path}")
        saved = torch.load(state_path, map_location=device, weights_only=False)
        for key, expected in identity.items():
            if saved.get(key) != expected:
                raise RuntimeError(f"VG014 resume mismatch for {key}")
        if saved.get("status") != "training":
            raise RuntimeError("VG014 can resume only active training state")
        ...
        if resume:
            raise RuntimeError("VG014 --resume requested without state")
    ...
    for epoch in range(completed_epoch + 1, config.epochs + 1):
        ...
        while True:
            ...
            for name in ("dagger1", "dagger2"):
                item = streams[name].next(model)
                if item is None:
                    raise RuntimeError(f"VG014 cyclic {name} stream ended")
                items[name] = item
            ...
            for _ in range(repetitions):
                ...
                with torch.autocast(
                    ...
                    loss, components = three_source_loss(
                ...
                for name in datasets:
                    source_weight_sums[name] += objective_weights[name]
            ...
            if set(final_outputs) != set(datasets):
                raise RuntimeError("VG014 emitted no complete source update")
            for name, item in items.items():
        ...
        validation_score = sum(
            objective_weights[name] * validation[name]["weighted_mse"]
            for name in datasets
        )
        ...
        if not weights_exact:
            raise RuntimeError("VG014 source objective weights diverged")
    ...
    if minimum_transition_exposure + 1e-12 < config.transition_window_exposure:
        raise RuntimeError("VG014 transition exposure contract failed")
    if not source_balance_audit:
        raise RuntimeError("VG014 source balance audit failed")
    selected_validation = (
        baseline_by_source
        if best_epoch == 0
        else history[best_epoch - 1]["validation"]
    )
    numerically_admitted = bool(
        ...
        < baseline_validation["dagger2"]["weighted_mse"]
