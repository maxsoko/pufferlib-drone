#!/usr/bin/env bash
# N108: bounded full-network six-gate aperture curriculum anchored to N104.
# Native-only; never connects to or resets official FlightSim.
set -euo pipefail
...
RUN_ROOT="${RUN_ROOT:-logs/drone_race_full_policy_six_gate_bootstrap/ppo_n108_anchored_aperture}"
...
python -m pufferlib.pufferl train drone_race_full_policy_six_gate_bootstrap \
    ...
    --eval-episodes 0 \
    --tag n108_six_gate_anchored_aperture \
    --seed 96 \
