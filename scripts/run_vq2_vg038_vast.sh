#!/usr/bin/env bash
# Source-locked Vast bootstrap and resumable VG038 higher-phase collection.
set -euo pipefail

VQ2_EXPECTED_COMMIT="${VQ2_EXPECTED_COMMIT:?set the pushed VG038 commit}"
VQ2_WORKSPACE_PATH="${VQ2_WORKSPACE_PATH:-/workspace/pufferlib-drone}"
VQ2_OUTPUT="${VQ2_OUTPUT:-$VQ2_WORKSPACE_PATH/logs/drone_race_full_policy_six_gate_bootstrap/vq2_vg038_variable_gate_dagger_round8_vg033_visited_512}"
VQ2_STATE="${VQ2_OUTPUT}_state.json"
VQ2_MANIFEST="$VQ2_WORKSPACE_PATH/docs/vq2_vg038_variable_gate_dagger_manifest_2026-07-31.json"
...
run_vg038_collection() {
    OMP_NUM_THREADS=4 MKL_NUM_THREADS=1 \
        python scripts/collect_vq2_vg038_variable_gate_dagger.py \
...
if [ ! -x .venv/bin/python ]; then
    echo "VG038 requires the source-locked Vast virtual environment" >&2
    exit 2
fi
source .venv/bin/activate

if [ -f "$VQ2_STATE" ]; then
    run_vg038_collection --resume
    exit $?
fi
...
if [ "$(nproc)" -lt 32 ]; then
    echo "VG038 requires at least 32 visible CPUs" >&2
    exit 2
fi
...
if [ "$VQ2_RAM_GIB" -lt 60 ]; then
    echo "VG038 requires approximately 64 GiB RAM" >&2
    exit 2
fi
if [ "$VQ2_DISK_FREE_GIB" -lt 20 ]; then
    echo "VG038 requires at least 20 GiB free" >&2
    exit 2
fi
...
if not torch.cuda.is_available():
    raise SystemExit("VG038 cannot see CUDA")
