#!/usr/bin/env python3
"""Refit VG022 on all five successive visited-state DAgger sources."""
...
TAG = "vq2_vg025_variable_gate_six_source_refit_001"
CHECKPOINT_SCHEMA = "vq2_variable_gate_recurrent_bc_checkpoint_v1"
REPORT_SCHEMA = "vq2_variable_gate_six_source_refit_report_v1"
STATE_SCHEMA = "vq2_variable_gate_six_source_refit_state_v1"
SEED = 429110
...
GOAL_PROMPT = three_source.GOAL_PROMPT
GOAL_PROMPT_SHA256 = three_source.GOAL_PROMPT_SHA256
PREREGISTRATION = (
    ROOT / "docs/vq2_vg025_six_source_refit_preregistration_2026-07-30.md"
)
RUNNER = ROOT / "scripts/run_vq2_vg025_vast.sh"
DEFAULT_OUTPUT = (
...
class SixSourceConfig:
    ...
    dagger1_objective_weight: float = 0.05
    dagger2_objective_weight: float = 0.05
    dagger3_objective_weight: float = 0.10
    dagger4_objective_weight: float = 0.15
    dagger5_objective_weight: float = 0.35
    ...
    maximum_dagger5_validation_weighted_mse: float = 0.10
...
def objective_weights(config: SixSourceConfig) -> dict[str, float]:
    return {
...
def six_source_loss(
    ...
    *,
    ...
    if total is None:
        raise RuntimeError("VG025 has no source objective")
    return total, components


def source_weight_audit(
    sums: dict[str, float], updates: int, config: SixSourceConfig
) -> bool:
    expected = {
...
def verify_inputs() -> None:
    ...
    for path, digest in expected.items():
        if sha256_path(path) != digest:
            raise RuntimeError(f"VG025 source evidence hash mismatch: {path}")
    if not PREREGISTRATION.is_file():
        raise RuntimeError("VG025 preregistration is missing")
    ...
    if (
        ...
        or parent.get("tag") != "vq2_vg022_five_source_device_decode_continuation_001"
        or not parent.get("completed")
        or not parent.get("numerically_admitted")
        or parent.get("best_epoch") != 9
        or parent.get("checkpoint_sha256") != PARENT_CHECKPOINT_SHA256
    ):
        raise RuntimeError("VG022 parent is not admitted for VG025")
    if (
        ...
        raise RuntimeError("VG022 admission evidence does not bind VG025")
    if (
        ...
        raise RuntimeError("recovered VG012 is not admitted for VG025")
    if (
        dagger3.get("schema")
    ...
    if (
        ...
        raise RuntimeError("VG016 admission evidence does not bind VG025")
    if (
        dagger4.get("schema")
    ...
    if (
        ...
        raise RuntimeError("VG019 admission evidence does not bind VG025")
    if (
        dagger5.get("schema")
    ...
    if (
        ...
        raise RuntimeError("VG024 admission evidence does not bind VG025")
...
def _load_parent(model: VQ2PhaseRecurrentActor) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("tag") != "vq2_vg022_five_source_device_decode_continuation_001"
        ...
        raise RuntimeError("VG022 parent actor contract changed")
...
def verify_completed_output(output: Path, report: dict[str, Any]) -> None:
    if (
        ...
        raise RuntimeError("existing VG025 report identity changed")
    ...
    if (
        ...
        raise RuntimeError("completed VG025 source/checkpoint identity changed")
    state = torch.load(
    ...
    if (
        ...
        raise RuntimeError("completed VG025 state does not bind the report")
...
def train(
    *,
    ...
    config: SixSourceConfig = SixSourceConfig(),
    resume: bool = False,
) -> dict[str, Any]:
    if config.sequence_chunk < 256 or config.transition_window_exposure < 4:
        raise RuntimeError("VG025 requires 256-step BPTT and 4x transitions")
    if objective_weights(config) != SOURCE_WEIGHTS:
        raise RuntimeError(
            "VG025 requires exact 0.30/0.05/0.05/0.10/0.15/0.35 weights"
        )
    verify_inputs()
    if device_name == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("VG025 preregisters CUDA training")
    ...
    if output.exists() and not state_path.is_file():
        raise RuntimeError("VG025 output exists without resumable state")
    ...
    if state_path.is_file():
        if not resume:
            raise FileExistsError(f"VG025 state already exists at {state_path}")
        saved = torch.load(state_path, map_location="cpu", weights_only=False)
        for key, expected in identity.items():
            if saved.get(key) != expected:
                raise RuntimeError(f"VG025 resume mismatch for {key}")
        if saved.get("status") != "training":
            raise RuntimeError("VG025 can resume only active training state")
        ...
        if resume:
            raise RuntimeError("VG025 --resume requested without state")
    ...
    for epoch in range(completed_epoch + 1, config.epochs + 1):
        ...
        while True:
            ...
            for name in ("dagger1", "dagger2", "dagger3", "dagger4", "dagger5"):
                item = streams[name].next(model)
                if item is None:
                    raise RuntimeError(f"VG025 cyclic {name} stream ended")
                items[name] = item
            ...
            for _ in range(repetitions):
                ...
                with torch.autocast(
                    ...
                    loss, components = six_source_loss(
            ...
            if set(final_outputs) != set(datasets):
                raise RuntimeError("VG025 emitted no complete source update")
            for name, item in items.items():
        ...
        if not weights_exact:
            raise RuntimeError("VG025 source objective weights diverged")
    ...
    if minimum_transition_exposure + 1e-12 < config.transition_window_exposure:
        raise RuntimeError("VG025 transition exposure contract failed")
    if not source_balance_audit:
        raise RuntimeError("VG025 source balance audit failed")
    selected_validation = (
    ...
    numerically_admitted = bool(
        ...
        < baseline_validation["dagger5"]["weighted_mse"]
